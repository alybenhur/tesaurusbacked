...
async getAuctionResults(
  @Param('gameId') gameId: string,
  @Query('onlyClosed') onlyClosed?: string  // ✅ NUEVO PARÁMETRO
): Promise<AuctionResultsResponseDto> {
  try {
    // ✅ Convertir string query param a boolean
    const requireClosed = onlyClosed === 'true';
    
    console.log(`📊 Getting auction results for game ${gameId}, requireClosed: ${requireClosed}`);
    
    return await this.auctionService.getAuctionResults(gameId, requireClosed);
  } catch (error) {

...
// ============================================================================
// ✅ ÍNDICES MODIFICADOS - Permite múltiples asignaciones de pistas por sponsor
// ============================================================================

// ✅ ÍNDICE 1: Un sponsor solo puede estar asignado UNA VEZ a una pista específica
// Esto previene: Sponsor A asignado dos veces a la misma pista en el mismo juego
GameSponsorAssociationSchema.index(
  { gameId: 1, sponsorId: 1, clueId: 1 }, 
  { 
    unique: true,
    partialFilterExpression: { clueId: { $ne: null } }  // Solo aplica cuando clueId existe
  }
);

// ✅ ÍNDICE 2: Un sponsor solo puede ser sponsor general UNA VEZ por juego
// Esto previene: Múltiples registros de "sponsor general" para el mismo sponsor en el mismo juego
GameSponsorAssociationSchema.index(
  { gameId: 1, sponsorId: 1 }, 
  { 
    unique: true,
    partialFilterExpression: { clueId: null }  // Solo aplica cuando clueId es null
  }
);

...
export class GameSponsorAssociationService {
  ...
  async createGameSponsorAssociation(createDto: CreateGameSponsorDto): Promise<GameSponsorAssociationDocument> {
    try {
      ...
      // ✅ CORRECCIÓN: Convertir todos los IDs a ObjectId explícitamente
      const gameSponsor = new this.gameSponsorModel({
        gameId: new Types.ObjectId(createDto.gameId),
        sponsorId: new Types.ObjectId(createDto.sponsorId),
        clueId: createDto.clueId ? new Types.ObjectId(createDto.clueId) : null,
        sponsorshipType: createDto.sponsorshipType || 'prize',
        sponsorshipAmount: createDto.sponsorshipAmount,
        sponsorshipDescription: createDto.sponsorshipDescription,
        startDate: createDto.startDate,
        endDate: createDto.endDate,
        // Nota: isActive, totalUnlocks y unlockedFor se inicializan con defaults del schema
      });
...
async getGameSponsors(gameId: string): Promise<GameSponsorAssociationDocument[]> {
  const filter = { 
    gameId: new Types.ObjectId(gameId)  // ✅ Ahora coincide con BD
  };
  ...
  return await this.gameSponsorModel
    ...
    .populate({
      path: 'clueId',
      options: { strictPopulate: false }  // ✅ CORRECCIÓN 2: Maneja nulls
    })
    .exec();
}
 ...
 */
// =====================================================
// MÉTODO NUEVO: Obtener juegos disponibles para un sponsor
// Agregar al archivo gamesponsor.service.ts
// =====================================================

/**
 * Obtener juegos disponibles para un sponsor
 * - Estado: 'waiting'
 * - Sin asociación previa con el sponsor
 */
async getAvailableGamesForSponsor(sponsorId: string): Promise<any[]> {
  try {
    // 1. Validar que el sponsorId sea válido
    if (!Types.ObjectId.isValid(sponsorId)) {
      throw new BadRequestException('Invalid sponsor ID format');
    }

    // 2. Buscar todos los juegos donde este sponsor YA está asociado
    const sponsorAssociations = await this.gameSponsorModel
      .find({ sponsorId: new Types.ObjectId(sponsorId) })
      .select('gameId')
      .exec();

    // Extraer los IDs de juegos donde ya está asociado
    const associatedGameIds = sponsorAssociations.map(assoc => assoc.gameId.toString());

    console.log(`📊 Sponsor ${sponsorId} ya está asociado a ${associatedGameIds.length} juegos`);

    // 3. Buscar juegos en estado 'waiting' que NO estén en la lista de asociados
    const availableGames = await this.gameModel
      .find({
        status: 'waiting',
        _id: { $nin: associatedGameIds.map(id => new Types.ObjectId(id)) }
      })
    ...
    console.log(`✅ ${availableGames.length} juegos disponibles encontrados para sponsor ${sponsorId}`);

    // ✅ 4. NUEVO: Buscar subastas activas para estos juegos
    const gameIds = availableGames.map(game => game._id);
    ...
    console.log(`🔨 ${auctions.length} subastas activas encontradas`);

    // Crear un mapa de gameId -> auction para acceso rápido
    const auctionMap = new Map();
    auctions.forEach(auction => {
      auctionMap.set(auction.gameId.toString(), auction);
    });

    // 5. Transformar la respuesta incluyendo información de subastas
    const gamesWithMetadata = availableGames.map(game => {
      const gameObj = game.toObject();
      const gameId = game._id.toString();
      
      // Buscar si hay subasta activa para este juego
      const auction = auctionMap.get(gameId);
      
      // ✅ Construir auctionInfo si existe subasta
      const auctionInfo = auction ? {
        ...
        totalBiddableClues: auction.biddableClues?.length || 0
      } : null;
      
      return {
        ...gameObj,
        // Metadata existente
        totalClues: gameObj.clues?.length || 0,
        hasClues: (gameObj.clues?.length || 0) > 0,
        availableForSponsorship: true,
        
        // ✅ NUEVO: Información de subasta
    ...
    console.error('❌ Error getting available games for sponsor:', error);
    throw error;
  }
}

/**
 * Obtener estadísticas de juegos disponibles para un sponsor
 */
async getAvailableGamesStatsForSponsor(sponsorId: string): Promise<{
  totalAvailable: number;
  totalClues: number;
  gamesWithClues: number;
  gamesWithoutClues: number;
}> {
  try {
    const availableGames = await this.getAvailableGamesForSponsor(sponsorId);

    const stats = {
      totalAvailable: availableGames.length,
      totalClues: availableGames.reduce((sum, game) => sum + (game.totalClues || 0), 0),
      gamesWithClues: availableGames.filter(game => game.hasClues).length,
      gamesWithoutClues: availableGames.filter(game => !game.hasClues).length,
    };

    return stats;
  } catch (error) {
    console.error('❌ Error getting available games stats:', error);
    throw error;
  }
}
